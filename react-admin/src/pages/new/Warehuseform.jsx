...
const Warehouseform = ({ inputs, title, value }) => {
  ...
  const addWarehouseHandler = (event) => {
    ...
    // console.log('sending ' + event.target.whHashAddress.value + ' to the whContract');
    warehouseContract.addWarehouse(event.target.whHashAddress.value);

      const requestOptions = {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            "whHashAddress":event.target.whHashAddress.value,
            "name":event.target.name.value,
            "email":event.target.email.value,
            "address":event.target.location.value,
            "password":event.target.whHashAddress.value,
            "role":'Warehouse'
            })
      };
          ...
          <div className="right">
            <form onSubmit={addWarehouseHandler}>
              <div className="formInput">
                ...
                <input id="whHashAddress" type="text" />
              </div>
              <div className="formInput">
